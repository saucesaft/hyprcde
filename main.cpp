...
Hyprlang::CParseResult onNewButton(const char* K, const char* V) {
    ...
    // hyprcde-button = color, size, icon, action
...
APICALL EXPORT PLUGIN_DESCRIPTION_INFO PLUGIN_INIT(HANDLE handle) {
    ...
    if (HASH != GIT_COMMIT_HASH) {
        HyprlandAPI::addNotification(PHANDLE, "[hyprcde] Failure in initialization: Version mismatch (headers ver is not equal to running hyprland ver)",
    ...
    HyprlandAPI::addConfigValue(PHANDLE, "plugin:hyprcde:bar_color", Hyprlang::INT{configStringToInt("rgba(33333388)")});
    HyprlandAPI::addConfigValue(PHANDLE, "plugin:hyprcde:bar_height", Hyprlang::INT{15});
    HyprlandAPI::addConfigValue(PHANDLE, "plugin:hyprcde:col.text", Hyprlang::INT{configStringToInt("rgba(ffffffff)")});
    HyprlandAPI::addConfigValue(PHANDLE, "plugin:hyprcde:bar_text_size", Hyprlang::INT{10});
    HyprlandAPI::addConfigValue(PHANDLE, "plugin:hyprcde:bar_title_enabled", Hyprlang::INT{1});
    HyprlandAPI::addConfigValue(PHANDLE, "plugin:hyprcde:bar_text_font", Hyprlang::STRING{"Sans"});
    HyprlandAPI::addConfigValue(PHANDLE, "plugin:hyprcde:bar_text_align", Hyprlang::STRING{"center"});
    HyprlandAPI::addConfigValue(PHANDLE, "plugin:hyprcde:bar_part_of_window", Hyprlang::INT{1});
    HyprlandAPI::addConfigValue(PHANDLE, "plugin:hyprcde:bar_precedence_over_border", Hyprlang::INT{0});
    HyprlandAPI::addConfigValue(PHANDLE, "plugin:hyprcde:bar_buttons_alignment", Hyprlang::STRING{"right"});
    HyprlandAPI::addConfigValue(PHANDLE, "plugin:hyprcde:bar_padding", Hyprlang::INT{7});
    HyprlandAPI::addConfigValue(PHANDLE, "plugin:hyprcde:bar_button_padding", Hyprlang::INT{5});

    HyprlandAPI::addConfigKeyword(PHANDLE, "hyprcde-button", onNewButton, Hyprlang::SHandlerOptions{});
    ...
    HyprlandAPI::addNotification(PHANDLE, "[hyprcde] Initialized successfully!", CColor{0.2, 1.0, 0.2, 1.0}, 5000);

    return {"hyprcde", "CDE-like window decorations for hyprland.", "saucesaft", "1.0"};
}

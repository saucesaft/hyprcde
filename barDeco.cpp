...
SDecorationPositioningInfo CHyprBar::getPositioningInfo() {
    static auto* const         PHEIGHT     = (Hyprlang::INT* const*)HyprlandAPI::getConfigValue(PHANDLE, "plugin:hyprcde:bar_height")->getDataStaticPtr();
    static auto* const         PPRECEDENCE = (Hyprlang::INT* const*)HyprlandAPI::getConfigValue(PHANDLE, "plugin:hyprcde:bar_precedence_over_border")->getDataStaticPtr();
...
void CHyprBar::onMouseDown(SCallbackInfo& info, IPointer::SButtonEvent e) {
    ...
    static auto* const PHEIGHT           = (Hyprlang::INT* const*)HyprlandAPI::getConfigValue(PHANDLE, "plugin:hyprcde:bar_height")->getDataStaticPtr();
    static auto* const PBARBUTTONPADDING = (Hyprlang::INT* const*)HyprlandAPI::getConfigValue(PHANDLE, "plugin:hyprcde:bar_button_padding")->getDataStaticPtr();
    static auto* const PBARPADDING       = (Hyprlang::INT* const*)HyprlandAPI::getConfigValue(PHANDLE, "plugin:hyprcde:bar_padding")->getDataStaticPtr();
    static auto* const PALIGNBUTTONS     = (Hyprlang::STRING const*)HyprlandAPI::getConfigValue(PHANDLE, "plugin:hyprcde:bar_buttons_alignment")->getDataStaticPtr();
    ...
    if (!VECINRECT(COORDS, 0, 0, assignedBoxGlobal().w, **PHEIGHT - 1)) {
        ...
        if (m_bDraggingThis) {
            ...
            Debug::log(LOG, "[hyprcde] Dragging ended on {:x}", (uintptr_t)PWINDOW.get());
        }
...
void CHyprBar::onMouseMove(Vector2D coords) {
    if (m_bDragPending) {
        ...
        Debug::log(LOG, "[hyprcde] Dragging initiated on {:x}", (uintptr_t)m_pWindow.lock().get());
...
void CHyprBar::renderBarTitle(const Vector2D& bufferSize, const float scale) {
    ...
    static auto* const PFONT             = (Hyprlang::STRING const*)HyprlandAPI::getConfigValue(PHANDLE, "plugin:hyprcde:bar_text_font")->getDataStaticPtr();
    static auto* const PALIGN            = (Hyprlang::STRING const*)HyprlandAPI::getConfigValue(PHANDLE, "plugin:hyprcde:bar_text_align")->getDataStaticPtr();
    static auto* const PALIGNBUTTONS     = (Hyprlang::STRING const*)HyprlandAPI::getConfigValue(PHANDLE, "plugin:hyprcde:bar_buttons_alignment")->getDataStaticPtr();
    static auto* const PBARPADDING       = (Hyprlang::INT* const*)HyprlandAPI::getConfigValue(PHANDLE, "plugin:hyprcde:bar_padding")->getDataStaticPtr();
    static auto* const PBARBUTTONPADDING = (Hyprlang::INT* const*)HyprlandAPI::getConfigValue(PHANDLE, "plugin:hyprcde:bar_button_padding")->getDataStaticPtr();
...
void CHyprBar::draw(CMonitor* pMonitor, float a) {
    ...
    static auto* const PCOLOR        = (Hyprlang::INT* const*)HyprlandAPI::getConfigValue(PHANDLE, "plugin:hyprcde:bar_color")->getDataStaticPtr();
    static auto* const PHEIGHT       = (Hyprlang::INT* const*)HyprlandAPI::getConfigValue(PHANDLE, "plugin:hyprcde:bar_height")->getDataStaticPtr();
    static auto* const PPRECEDENCE   = (Hyprlang::INT* const*)HyprlandAPI::getConfigValue(PHANDLE, "plugin:hyprcde:bar_precedence_over_border")->getDataStaticPtr();
    static auto* const PALIGNBUTTONS = (Hyprlang::STRING const*)HyprlandAPI::getConfigValue(PHANDLE, "plugin:hyprcde:bar_buttons_alignment")->getDataStaticPtr();
    static auto* const PENABLETITLE  = (Hyprlang::INT* const*)HyprlandAPI::getConfigValue(PHANDLE, "plugin:hyprcde:bar_title_enabled")->getDataStaticPtr();
...
uint64_t CHyprBar::getDecorationFlags() {
    static auto* const PPART = (Hyprlang::INT* const*)HyprlandAPI::getConfigValue(PHANDLE, "plugin:hyprcde:bar_part_of_window")->getDataStaticPtr();
...
void CHyprBar::applyRule(const SWindowRule& r) {
    ...
    if (r.szRule == "plugin:hyprcde:nobar")
        m_bHidden = true;
    else if (r.szRule.starts_with("plugin:hyprcde:bar_color"))
        m_bForcedBarColor = CColor(configStringToInt(arg));
    else if (r.szRule.starts_with("plugin:hyprcde:title_color"))
